...
session_start();
$orderQuery = mysqli_query($conn2, "SELECT * FROM orders WHERE status='in progress'");
...
if (isset($_POST['orderIDDeliverNow'])) {

    $id = $_POST['orderIDDeliverNow'];
    $method = $_POST["method"];
    ...
    if ($checkFetch['status'] == "delivered") {
        $warning_msg[] = 'Already Delivered';
    } else if ($checkFetch['status'] == "on going") {
        $warning_msg[] = "Already on going";
    } else if ($checkFetch['status'] == "in progress") {
        // update to on going
        if ($method == "cash") {
            mysqli_query($conn2, "UPDATE orders SET status='on going' WHERE id='$id'");
        }
        if ($method == "paymaya") {
            mysqli_query($conn2, "UPDATE orders SET status='delivered' WHERE id='$id'");
        }
        $success_msg[] = 'Successfully on going. Please deliver now.';
    } else {
        $checkFetch['status'] == "Error";
    }
    ...
    <table>
        ...
        <?php while ($fetchOrder = mysqli_fetch_array($orderQuery)) { ?>
            ...
            <tr>
                ...
                <td>
                    <?php if ($fetchOrder['transaction_status'] == "in progress") { ?>
                    ...
                    <?php } else { ?>
                        ...
                        >
                            <input
                                type="hidden"
                                name="orderIDDeliverNow"
                                value="<?php echo $fetchOrder['id']; ?>"
                            >
                            <input
                                type="hidden"
                                name="method"
                                value="<?php echo $fetchOrder['method']; ?>"
                            >
                            <button
                                type="submit"
                                name="deliverNow"
...
<script>
let deliverNow = $('.deliverNow');

deliverNow.each(function() {
    $(this).on('click', function() {
        let getOrderID = $(this).parent().parent().find(":nth-child(1)").text();
        ...
        $.ajax({
            ...
            success: (deliverNowRespo) => {
                alert(deliverNowRespo)
            }
