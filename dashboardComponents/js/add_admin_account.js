$(document).ready(() => {
  $(".submitRegister").on("click", () => {
    ...
    if (
      ...
      nameRegister.val().length > 29 ||
      nameRegister.val().length < 3
    ) {
      ...
      usernameRegister.val().length > 19 ||
      usernameRegister.val().length < 4
    ) {
      ...
      passRegister.val().length > 100 ||
      passRegister.val().length < 6
    ) {
      ...
      addressRegister.val().length > 90 ||
      addressRegister.val().length < 6
    ) {
    ...
    } else if (selectRegister.val() !== "Admin") {
      ...
      $.ajax({
        method: "POST",
        url: "./dashboardActions/add_admin_accountActions.php",
        data: {
